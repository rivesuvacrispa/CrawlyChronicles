...
// Credits: LazySquirrelLabs (https://github.com/lazysquirrellabs/min_max_range_attribute)
// Edits: Added support for custom fields

[CustomPropertyDrawer(typeof(Util.Attributes.MinMaxRangeAttribute))]
internal class MinMaxRangeDrawer : PropertyDrawer
{
	...
	{
		if (attribute is not Util.Attributes.MinMaxRangeAttribute minMaxAttribute)
		{
		...
		/*
		 * @author RivesUvaCrispa
		 */
		
		else if (property.propertyType == SerializedPropertyType.Generic &&
		           property.boxedValue?.GetType() == typeof(LevelInt))
		{
			
			LevelInt levelInt = (LevelInt) property.boxedValue;
			
			var value = levelInt.Value;
			var minValue = (float)value.x;
			var maxValue = (float)value.y;
			(minValue, maxValue) = ClampValues(minValue, maxValue, minLimit, maxLimit);
			DrawSlider(position, property, minLimit, maxLimit, ref minValue, ref maxValue, BuildIntLabel);
			...
			property.boxedValue = new LevelInt(value);
			
		} else if (property.propertyType == SerializedPropertyType.Generic &&
		           property.boxedValue?.GetType() == typeof(LevelFloat))
		{
			
			LevelFloat levelFloat = (LevelFloat) property.boxedValue;
			
			var value = levelFloat.Value;
			var minValue = value.x;
			var maxValue = value.y;
			(minValue, maxValue) = ClampValues(minValue, maxValue, minLimit, maxLimit);
			DrawSlider(position, property, minLimit, maxLimit, ref minValue, ref maxValue, BuildFloatLabel);
			...
			property.boxedValue = new LevelFloat(value);
		}
